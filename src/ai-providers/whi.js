/**
 * whi.js
 * AI provider implementation for whi using OpenAI-compatible API.
 */

import { createOpenAI } from '@ai-sdk/openai';
import { BaseAIProvider } from './base-provider.js';

export class WhiProvider extends BaseAIProvider {
    constructor() {
        super();
        this.name = 'whi';
    }
    ...
    /**
     * Creates and returns a whi client instance.
     * @param {object} params - Parameters for client initialization
     * @param {string} params.apiKey - whi API key
     * @param {string} [params.baseURL] - Optional custom API endpoint
     * @returns {Function} whi client function
    ...
    getClient(params) {
        try {
            ...
            if (!apiKey) {
                throw new Error('whi API key is required.');
            }

            // 使用配置的API端点，可以通过baseURL参数覆盖
            const effectiveBaseURL = baseURL || 'https://api.whi.com';
    ...
    /**
     * Maps whi model IDs to actual API model names
     * @param {string} modelId - The model ID from supported-models.json
     * @returns {string} The actual model name to use with the API
     */
    mapModelId(modelId) {
        // 自动去除whi-前缀，类似poloai的处理方式
        if (modelId.startsWith('whi-')) {
            return modelId.replace('whi-', '');
        }
    ...
    /**
     * Generates text using whi with model ID mapping
     */
    ...
    /**
     * Generates streaming text using whi with model ID mapping
     */
    ...
    /**
     * Generates object using whi with model ID mapping
     */
