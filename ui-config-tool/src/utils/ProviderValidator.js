...
export class ProviderValidator {
    ...
    validateProvider(provider) {
        ...
        // Type validation
        const validTypes = ['openai', 'anthropic', 'google', 'polo', 'foapi', 'openrouter', 'custom'];
    ...
    async testAnthropicAPI(provider) {
        try {
            ...
            if (response.ok || response.status === 400) { // 400 might be expected for test message
                return {
                    isValid: true,
                    errors: [],
                    message: `✅ ${provider.name} API连接成功`,
    ...
    async testCustomAPI(provider) {
        try {
            ...
            return {
                ...
                message: response.ok ? `✅ ${provider.name} 连接成功` : `❌ ${provider.name} 连接失败`,
                details: {
                    status: response.status,
